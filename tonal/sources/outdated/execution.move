...
//     public struct ProgrammableTransaction {
//         safe: ID,
//         transaction: ID,
...
//     public fun execute_management(safe: &mut Safe, storage: &mut Storage, transaction: &mut Transaction, clock: &Clock, ctx: &mut TxContext) {
...
//     public fun request_objects_transfer(safe: &Safe, transaction: &Transaction, treasury: &Treasury, clock: &Clock, ctx: &TxContext): ObjectsTransfer {
...
//     public fun request_coins_transfer(safe: &Safe, transaction: &Transaction, treasury: &Treasury, clock: &Clock, ctx: &TxContext): CoinsTransfer {
...
//     public fun request_ptb_execution(safe: &Safe, transaction: &Transaction, clock: &Clock, ctx: &TxContext): ProgrammableTransaction {
//         assert!(safe.is_valid_owner_cap(owner_cap, ctx), EInvalidOwnerCap);
//         assert_transaction_readiness(safe, transaction, clock);
...
//     public fun transfer_object<T: key + store>(request: &mut ObjectsTransfer, treasury: &mut Treasury, index: u64) {
...
//     public fun transfer_coin<T>(request: &mut CoinsTransfer, treasury: &mut Treasury, index: u64, ctx: &mut TxContext) {
//         assert!(request.treasury == treasury.id(), ERequestTreasuryMismatch);
...
//     public fun confirm_objects_transfer(request: ObjectsTransfer, transaction: &mut Transaction, clock: &Clock, ctx: &TxContext) {
...
//     public fun confirm_coins_transfer(request: CoinsTransfer, transaction: &mut Transaction, clock: &Clock, ctx: &TxContext) {
...
//     public fun confirm_programmable_transaction(request: ProgrammableTransaction, transaction: &mut Transaction, clock: &Clock, executed_commands: vector<u64>, ctx: &TxContext) {
//         let ProgrammableTransaction { safe: _, transaction: _, commands, inputs: _ } = request;
